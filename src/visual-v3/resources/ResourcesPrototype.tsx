...
import {ArrowRight, Bookmark, Clock3, Compass, ExternalLink, Menu, PanelLeftClose, PanelLeftOpen, Search, Shuffle, X} from 'lucide-react';
...
export default function ResourcesPrototype() {
  ...
  return (
    <V3Frame area="resources" title="资源原型">
      <div className={styles.resources} data-index-collapsed={indexCollapsed}>
        <aside className={styles.sideNav} data-open={menu} data-collapsed={indexCollapsed}>
          <div className={styles.archiveMark}>
            <b>CR.</b>
            <span>研究索引<br />RESEARCH INDEX</span>
            <button type="button" onClick={() => setIndexCollapsed(!indexCollapsed)} aria-label={indexCollapsed ? '展开研究索引' : '收起研究索引'} title={indexCollapsed ? '展开研究索引' : '收起研究索引'}>
              {indexCollapsed ? <PanelLeftOpen size={15} /> : <PanelLeftClose size={15} />}
            </button>
          </div>
          <nav>
            {['精选', '论文', '算法', '压缩器', '教程', '数据集', '研究图谱', '技术脉络'].map((item, index) => <a key={item} title={item} href={index < 2 ? '#精选' : index < 6 ? '#论文' : '#技术脉络'}><span>{String(index + 1).padStart(2, '0')}</span><b>{item}</b></a>)}
          </nav>
          <button type="button" onClick={randomPaper} title="随机探索"><Shuffle size={15} /><span>随机探索</span></button>
